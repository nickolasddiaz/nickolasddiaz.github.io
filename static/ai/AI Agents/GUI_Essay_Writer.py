...
class WriterGui:
    ...
    def run_agent(self, start, topic, stop_after):
        ...
        while self.iterations[self.thread_id] < self.max_iterations:
            ...
            ## fix
            lnode, nnode, _, rev, account = self.get_disp_state()
            yield self.partial_message, lnode, nnode, self.thread_id, rev, account
            config = None  # need
    ...
    def find_config(self, thread_ts):
        for state in self.graph.get_state_history(self.thread):
            config = state.config
            if  'thread_ts' in config['configurable'] and config['configurable']['thread_ts'] == thread_ts:
                return config
        return None

    def copy_state(self, hist_str):
        """ result of selecting an old state from the step pull down. Note does not change thread.
            ...
            def updt_disp():
                ...
                for state in self.graph.get_state_history(self.thread):
                    if state.metadata['step'] < 1:  # ignore early states
                        continue
                    if 'thread_ts' == state.config['configurable']:
