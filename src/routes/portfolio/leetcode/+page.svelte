<script lang="ts">
	...
	let submissions: SubmissionData = $state({
	    "html_content": "Awaiting code",
		"submissions": [{
        "time": 0,
        "language": "py",
        "runtime": 0,
        "memory": 0.0,
        "accepted": true,
        "code_content": "Awaiting code"
		}]
	});
	const cache = new Map<string, SubmissionData>();

	
	async function loadSubmission(id: string) {
   	  if (cache.has(id)) {
          submissions = cache.get(id)!;
          return;
      }
      try {
          const res = await fetch(`/leetcode/${id}.json`);
...
<MainContent>
    <p class="paragraph-text">Here serves all of my {Object.keys(data).length} Leetcode Submissions. <a class="link-primary" title="Leetcode" href="https://leetcode.com/u/nickolasddiaz/">Leetcode Link</a></p>
        ...
        <Tabs tabStyle="underline">
          <TabItem open>
            ...
            <p class="text-gray-500 dark:text-gray-400">
                {@html submissions["html_content"]}
            </p>
          </TabItem>
          {#each submissions["submissions"] as item (item["code_content"])}
              <TabItem>
                  {#snippet titleSlot()}
                    <div class="flex items-center gap-2">
