...
import type ShapeCreator from "./ShapeCreator.js";
...
type color_ids = [HTMLButtonElement, HTMLButtonElement,HTMLButtonElement,HTMLButtonElement,HTMLButtonElement]

export default class Main {
    webGL: any;
    program: WebGLProgram;
    color: color = [0,0,0] as color;
    shapes: Triangle[];
    gl: WebGLRenderingContext;
    isLine: boolean = false;
    positionAttrib: number;
    colorAttrib: number;
    shape_selector: number = 0;
    isPolyGon: boolean = false;
    prev_color_id: HTMLButtonElement;

    constructor(gl: WebGLRenderingContext, canvas: HTMLCanvasElement, 
        shape_id: HTMLSelectElement, draw_id: HTMLSelectElement, color_ids: color_ids) {
        this.gl = gl;
        this.prev_color_id = color_ids[0]

        this.webGL = new InitWebGLProgram(gl);
        let vertexShader = this.webGL.createShader(
            gl.VERTEX_SHADER,
            vertexShaderSource
        );
        let fragmentShader = this.webGL.createShader(
            gl.FRAGMENT_SHADER,
    ...
    hover_clickeventlistener(canvas: HTMLCanvasElement){
    ...
    left_clickeventlistener(canvas: HTMLCanvasElement){
        const rect = canvas.getBoundingClientRect();
    ...
    right_clickeventlistener(canvas: HTMLCanvasElement){
    ...
    color_eventlistener(color_ids: color_ids): void{
    ...
    style_eventlistener(draw_id: HTMLSelectElement): void {
    ...
    shape_eventlistener(shape_id: HTMLSelectElement): void {
        shape_id.addEventListener("change", (event) => {
            this.shape_selector = Number((event.target as HTMLSelectElement).value);
            this.isPolyGon = this.shape_selector == 4 || this.shape_selector == 1;
            this.shapes[this.shapes.length - 1] = this.shape_creator();
        });
    }
    hexToWebGLColor(hex: string): color {
