...
export default class Player extends GameObject {
    ...
    bullet_cooldown;
    gl;
    constructor(gl, size, start_position, add_object) {
    ...
    update(deltatime) {
        if (this.bullet_cooldown && this.keysPressed[' ']) {
            const bullet = new Bullet(this.gl, 1, [this.transform[0] + Math.cos(this.rotation[2]) / 10, (this.transform[1] + Math.sin(this.rotation[2]) / 10) - 1.5, this.transform[2]], [this.rotation[0], this.rotation[1], this.rotation[2]]);
            bullet.go_forward(5);
