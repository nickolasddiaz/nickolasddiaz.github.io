...
export default class Player extends GameObject{
    ...
    constructor(gl: WebGLRenderingContext, program: WebGLProgram,
        ...
        size: vec3, start_position: vec3,
        add_object: (object: GameObject) => void, loose: () => void,
    ){
        super(gl, program, size, start_position, [.2, .2, Math.random() * TAU],
            GameObjectEnum.PLAYER, mScale, mRotation, mTransform,
    ...
    update(deltatime: number): void {
        if (this.bullet_cooldown && this.keysPressed[' ']){
            const bullet = new Bullet(this.gl, this.program, this.mScale, this.mRotation ,this.mTransform,
                [.025, .025, .025], [this.transform[0] + Math.cos(this.rotation[2]) / 10, this.transform[1] + Math.sin(this.rotation[2]) / 10, this.transform[2]], 
